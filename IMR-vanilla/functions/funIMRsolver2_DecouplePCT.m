function [t,P,T,C,tdel,Tdel,Cdel] = funIMRsolver2_DecouplePCT(model,tspan,R0,NT,NTM, ...
    Pext_type,Pext_Amp_Freq,disptime,Tgrad,Tmgrad,Cgrad,Dim,comp,Rfit_pp,Rfit_pp_d1,Rfit_pp_d2,t2Start,eqR)

% Authors:
% Jin Yang
% jyang526@wisc.edu
% UW Madison Mechanical Engineering, Postdoc

% last Update: 8/21/2017, 10/18/2018
...
%***************************************
% Load Parameters :
Pmt = IMRcall_parameters(R0,1,1,1); % Calls parameters script
...
[t, X] = ode23tb(@bubble_Rknown, [0 tspan_star] , X0,options);

P = X(:,1); % Internal pressure
Tau = X(:,2:(NT+1)); % Variable relating to internal temp
C =  X(:,(NT+2):(2*NT+1)); % Vapor concentration in the bubble
% S = X(:,2); % Stress integral
% Tm = X(:, (2*NT+3):end ); % Temperature variation in the medium
% ====== JY!!! Tm and S will solve later ====== 
% S0 = 0;
% Tm0 = ones(1,NTM); % Temperature variation in the medium
% [t, Tm] = ode23tb(@bubble_Tm, [0 tspan_star], Tm0);
...
% This function has acess to all parameters above
    ...
    function dxdt = bubble_Rknown(t,x)
        
        % Break x vector into indv. values
        R = func_Rfit(t*t0+t2Start)/R0; % Bubble wall Radius
        U = func_d1Rfit(t*t0+t2Start)/(R0/t0); % Bubble wall velocity
        P = x(1); % Internal pressure
        %Z1 = x(4); % Stress integral
        
        % ====== JY!!! S is determined with known R ======
        % S = x(2);
        % Tau = x(3:(NT+2));
        % C = x((NT+3):(2*NT+2));
        % Tm = x((2*NT+3):end);
        ...
        Tau = x(2:(NT+1));
        C = x((NT+2):(2*NT+1));
        % ====== JY!!! S and Tm will solve later ======
        S = 0;
        Tm = ones(NTM,1);
         
        
        waitbar(t/tspan_star);
        if (disptime == 1)
            disp(t/tspan_star);
        end
...
%         Sdot = Sdot - SdotA*udot/R;
        ...
        % dxdt = [pdot; Sdot; Tau_prime; C_prime; Tm_prime];
        dxdt = [pdot; Tau_prime; C_prime];
...
% throughout the code
    function R = func_Rfit(t)
        R = ppval(Rfit_pp,t);
    end
        
    function U = func_d1Rfit(t)
        U = ppval(Rfit_pp_d1,t);
    end
